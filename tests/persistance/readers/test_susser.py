...
def test_reads_complete_grid():
    ...
    # ASSERT
    assert all(grid._puzzle_digits[idx] == int(char) for idx, char in enumerate(text))
...
def test_reads_digits_into_correct_cells():
    ...
    assert all(digit == 0 for digit in grid._puzzle_digits[1:])
...
def test_leaves_empty_cells_unset():
    ...
    # ASSERT
    assert all(val == 0 for val in grid._puzzle_digits)
...
def test_supports_preferred_empty_characters(empty):
    ...
    # ASSERT
    assert grid._puzzle_digits[0] == 5
    assert all(digit == 0 for digit in grid._puzzle_digits[1:])
...
def test_digits_are_written_to_correct_cells():
    # ARRANGE
    text = (
        "1........"
        "........."
    ...
    # ASSERT
    assert grid._puzzle_digits[Cell(0, 0).index] == 1
    assert grid._puzzle_digits[Cell(4, 4).index] == 5
    assert grid._puzzle_digits[Cell(8, 8).index] == 9

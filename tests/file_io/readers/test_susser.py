...
def test_reads_complete_grid():
    ...
    # ASSERT
    assert all(grid.values[idx] == int(char) for idx, char in enumerate(text))
...
def test_reads_values_into_correct_cells():
    ...
    assert all(value == 0 for value in grid.values[1:])
...
def test_leaves_empty_cells_unset():
    ...
    # ASSERT
    assert all(val == 0 for val in grid.values)
...
def test_values_are_written_to_correct_cells():
    ...
    # ASSERT
    assert grid.values[Cell(0, 0).index] == 1
    assert grid.values[Cell(4, 4).index] == 5
    assert grid.values[Cell(8, 8).index] == 9
